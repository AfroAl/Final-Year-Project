...
import fyp_project.detector.patterns.*;;

public class SecurityMatcher
{
    private HashMap<Integer , Pattern> patterns = new HashMap<>();
    private String[] keys = {"SQL Injection - prepareStatement", "SQL Injection - executeQuery", "SQL Injection - createQuery", "SQL Injection - parameterisedString", 
    						 "LDAP - Unsafe Search", "RegEx - ReDOS Prevention", "XSS - Input Validation", "Sensitive Data Exposure - Insecure Cryptography",
    ...
    private String[] desc = { "<html><body><p><b><u>SQL Injection - prepareStatement</u></b></p> <br> <p><b>Exploitability: 3\u00A0\u00A0\u00A0\u00A0Prevalence: 2\u00A0\u00A0\u00A0\u00A0Detectability: 3\u00A0\u00A0\u00A0\u00A0Technical: 3</b></p> <br> <p>A SQL injection attack consists of insertion or “injection” of a SQL query via the input data from the client to the application. A successful SQL injection exploit can read sensitive data from the database, modify database data (Insert/Update/Delete), execute administration operations on the database (such as shutdown the DBMS), recover the content of a given file present on the DBMS file system and in some cases issue commands to the operating system. SQL injection attacks are a type of injection attack, in which SQL commands are injected into data-plane input in order to effect the execution of predefined SQL commands.</p> <br> <p>To prevent SQL Injections we must write parameterized queries. To create perameterised query in java we have PreparedStatement. It can take parameters by passing question marks (?) in the query and then by replacing each question mark index with required values.</p></body></html>",
    						  ...
    						  "<html><body><p><b><u>SQL Injection - parameterisedString</u></b></p> <br> <p><b>Exploitability: 3\u00A0\u00A0\u00A0\u00A0Prevalence: 2\u00A0\u00A0\u00A0\u00A0Detectability: 3\u00A0\u00A0\u00A0\u00A0Technical: 3</b></p> <br> <p>A SQL injection attack consists of insertion or “injection” of a SQL query via the input data from the client to the application. A successful SQL injection exploit can read sensitive data from the database, modify database data (Insert/Update/Delete), execute administration operations on the database (such as shutdown the DBMS), recover the content of a given file present on the DBMS file system and in some cases issue commands to the operating system. SQL injection attacks are a type of injection attack, in which SQL commands are injected into data-plane input in order to effect the execution of predefined SQL commands.</p> <br> <p>To prevent SQL Injections we must write parameterized queries. To create perameterised query in java we have PreparedStatement. It can take parameters by passing question marks (?) in the query and then by replacing each question mark index with required values.</p></body></html>",
    ...
    public SecurityMatcher(ArrayList<String> lines) {
        ...
        patterns.put(3, ParameterisedStringPattern.getPattern());

        //LDAP Injection
        patterns.put(4, LDAPPattern.getPattern());

        //RegEx Injection
        patterns.put(5, RegExPattern.getPattern());

        //XSS
        patterns.put(6, XSSPattern.getPattern());

        //Sensitive Data Exposure
        patterns.put(7, CryptoKeysPattern.getPattern());
        patterns.put(8, WebServerMainPattern.getPattern());
        patterns.put(9, Base64EncoderPattern.getPattern());
        patterns.put(10, InsecureRNGPattern.getPattern());
    }
    ...
    public String[] patternMatch(String line, int i, int patternNumber, int patternValue) {
        ...
        if(patternValue == 1 && patternNumber > 5) {
            return results;
        } else if(patternValue  == 2 && patternNumber != 6) {
            return results;
        } else if(patternValue == 3 && patternNumber < 7) {
            return results;
        } else if(patternValue == 4 && patternNumber == 6) {
            return results;
        } else if(patternValue == 5 && patternNumber < 6) {
            return results;
        }
        ...
        if(m.find()) {
            ...
            if(patternNumber == 0 || patternNumber == 1 || patternNumber == 2 || patternNumber == 3) {
            ...
            if(patternNumber < 4 && line.contains("?")) {
                ...
                skip = true;
            } else if( patternNumber == 5 && (count < 2 || line.contains("// Checked"))) {
                skip = true;
            } else if(patternNumber == 6 && line.contains("// Checked")) {
                skip = true;
            } else if(patternNumber == 7 && line.contains("// Safe"))  {
                skip = true;
            } else if(patternNumber == 8 && !isServlet) {
                skip = true;
            } else if(patternNumber == 8 && line.contains("// Unsafe")) {
                skip = true;
            } else if(patternNumber == 9 && line.contains("// Unsafe")) {
                skip = true;
            }
